...
int main(int argc, char** argv){
	...
	if (DBG == 1) {
		char str[20] = "result_test/result_";
		char buf[3];
		sprintf(buf, "%d", M);
		strcat(str, buf);
		strcat(str, ".txt");
		fichero = fopen(str, "wt");
		fprintf(fichero, "ID \t MULT(SW) \t MULT(HW) \t ACEL. MULT. \t ENC(SW) \t ENC(HW) \t ACEL. ENC. \n");
	}
	...
	for (int n_test = 1; n_test <= N_TEST; n_test++) {
		...
		if (DBG == 1) {
			fprintf(fichero, "%d \t %6llu \t %6llu \t %f \t %6llu \t %6llu \t %f \n", n_test, tiempo_mult_3round, tiempo_mult_hw, (float)tiempo_mult_3round / (float)tiempo_mult_hw, tiempo_enc_3round, tiempo_enc_hw, (float)tiempo_enc_3round / (float)tiempo_enc_hw);
		}
